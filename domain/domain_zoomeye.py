...
def get_accesstoken_zoomeye(domain):
    ...
    if "error" in responsedata and responsedata['error'] == "bad_request":
        return False
    access_token1 = responsedata.get('access_token',
                                     "78c4a2dd70ddeffc5fc3c0639f86245a")
...
def search_zoomeye(domain):
    ...
    req = requests.get('http://api.zoomeye.org/web/search/?query=site:%s&page=1'
                       % domain, headers=authData)
...
def output(data, domain=""):
    if type(data) == list and data[1] == "INVALID_API":
        print(colored(style.BOLD + '\n[-] ZoomEye username and password not \
                      configured. Skipping Zoomeye Search.\nPlease refer to \
                      http://datasploit.readthedocs.io/en/latest/apiGeneration/.'
                      + style.END, 'red'))
        print("")
    elif type(data) == list and data[1] == "BAD_API":
        print(colored(style.BOLD + '\n[-] ZoomEye API is not functional right \
                      now.\n' + style.END, 'red'))
    else:
        data = data[1]
        if data['available'] > 0:
            for site in data['matches']:
                if site['site'].split('.')[-2] == domain.split('.')[-2]:
                    location = site['geoinfo'] if 'geoinfo' in site else ''
                    print("Site:\n    {}".format(site['site']))
                    print("Title:\n    {}".format(site['title']))
                    print("IP(s):\n    {}".format('\n    '.join(map(str, site['ip']))))
                    print("Headers:\n    {}".format(site['headers'].replace("\r\n", "\n    ").rstrip('\n    ')))
                    print("Geographic Information:")
                    print("    Continent: {} ({})".format(location['continent']['names']['en'],
                                                          location['continent']['code']))
                    print("    Country: {} ({})".format(location['country']['names']['en'],
                                                        location['country']['code']))
                    print("    City: {}".format(location['city']['names']['en']))
                    print("    Latitude: {}".format(location['location']['lat']))
                    print("    Longitude: {}".format(location['location']['lon']))
                    print("    ISP: {} ({})".format(location['isp'],
